...
enum EHPMError
{
	...
	EHPMError_InvalidDashboardPageLayout,				// The dashboard page layout was invalid. Ensure the number of columns matches the layout type.
	EHPMError_InvalidDashboardConfiguration,			// The dashboard configuration was invalid. Additional information might be available through @{UtilGetLastErrorDescription}.
	EHPMError_UserNotMemberOfProject,					// The SDK user running the command is not member of the project that the command requested. See also @{HPMResourceProperties}::m_SDKSyncMethod.
	EHPMError_DefaultQAWorkflowCannotBeDeleted,			// The default QA workflow cannot be deleted.
	EHPMError_DefaultQAWorkflowMustBeVisibleInQA,		// The default QA workflow must be visible in the QA section.
};
...
|	Index:				!name																		|
\*_________________________________________________________________________________________________*/
typedef struct HPMBinaryBuffer
{
	HPMUInt32 m_nBytes;			// The number of bytes of data.
	const HPMUInt8 *m_pBytes;	// The binary data.
} HPMBinaryBuffer;
...
typedef struct HPMFindContext
{
	...
	HPMInt32 m_bSeparateSubProjectPath;				// [default=1] If set to 1 subproject path is included in find.
} HPMFindContext;
...
typedef struct HPMReportEmailProperties
{
	...
	HPMInt32 m_HourOfDay;							// [default=12] The number of half hours past midnight to send project reports. 0 -> 47 (number of half hours from midnight).
...
typedef struct HPMReport
{
	...
	HPMUInt32 m_nColumnsOrdering;									// Number of columns in the order they should be shown for this report.
	const HPMColumn *m_pColumnsOrdering;							// Pointer to a list of columns in the order they should be shown for this report. See @{HPMColumn}.
